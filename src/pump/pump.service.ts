...
export class PumpService {
  ...
  async createToken(
    ...
    try {
      ...
      // Derive creator vault PDA manually to verify
      ...
      const fileBlob = fileBuffer && fileBuffer.length > 0 ? new Blob([fileBuffer]) : new Blob();
      ...
      // Reduced compute budget to avoid conflicts
      const computeBudgetOpts = { 
        unitLimit: 300000, 
        unitPrice: 1000000 
      };

      // Try createAndBuy with better error handling
  ...
  // Alternative method: Create token without immediate buy (using smaller buy amount)
  async createTokenOnly(data: CreateTokenDto, imageFile?: Buffer): Promise<any> {
    ...
    try {
      ...
      const fileBlob = imageFile && imageFile.length > 0 ? new Blob([imageFile]) : new Blob();
      ...
      // Use minimal buy amount instead of create-only
      const minimalBuyLamports = BigInt(Math.floor(0.0001 * LAMPORTS_PER_SOL)); // 0.0001 SOL
      const slippage = 500n;
  ...
  // Debug method to verify PDA derivation
